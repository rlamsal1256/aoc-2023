...
def convert_spelled_number(input_value):
    ...
    return spelled_out_numbers.get(input_value.lower())

def find_spelled_out_numbers(text):
    ...
    for i in range(len(text)):
        for j in range(i + 1, len(text) + 1):
            substring = text[i:j]
            if substring.isdigit() or substring in spelled_out_numbers:
                results.append(substring)

    return int(f"{convert_spelled_number(results[0])}{convert_spelled_number(results[-1])}")
...
#     '7pqrstsixteen',
# ]
total_combined_alphanumerics = 0

for word in word_list:
    alphanumerics = find_spelled_out_numbers(word)
    print(f"'{word}': {alphanumerics}")
    total_combined_alphanumerics += int(alphanumerics)

print(f"\nTotal combined alphanumeric: {total_combined_alphanumerics}")
