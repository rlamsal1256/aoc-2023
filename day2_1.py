...
def possible1(item):
    ...
    # print({color: int(count)})
    return max[color] >= int(count)

def possible2(s):
    set = s.strip().split(',')
    return all(possible1(item) for item in set)
...
for line in input:
    ...
    g, n = game.strip().split(' ')
    sets = rest.strip().split(';')
    # print(f"{n}: {all(possible2(set) for set in sets)}")
    if all(possible2(set) for set in sets):
        sum += int(n)
